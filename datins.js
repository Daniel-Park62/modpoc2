...
let MEAS = 5;
let svtime = moment().subtract(32,"s");
let lastupd = moment().subtract(32,"s");
...
//        if ( rapdev[i] == 0) continue ;
        ...
        con.query("UPDATE motestatus SET MAC = ?, ACT = ? , BATT = case when gubun <> 'S' then ? else batt end, LOWCNT = LOWCNT + ? where seq = ?",[motestatus.mac, motestatus.act, motestatus.batt,  lowcnt, motestatus.seq ],
...
async function  createData(sgno, today,uint16View) {
    ...
    // console.log("chk sv = :" ,sgno, svtime.format('YYYY-MM-DD HH:mm:ss.S')) ;
    await ( () => {
      for(let i=0 ; i < 350 ; i++) {
        if ((i % 35) == 0) {
          // today = moment(uint16View[i] , "X") ;
          console.log("chk = :" + moment(uint16View[i] , "x")) ;
          continue ;
        }
        if ((i % 35) == 1) {

          con.query('UPDATE motestatus SET BATT = ? where sensorNo = ?',[ uint16View[i] / 100.0 , sgno],
           (err, res) => { if (err) console.error("Update motestatus :", err); }
          );
          continue ;
        }
...
setTimeout( getMeasure, 2000) ;

setTimeout( main2_loop,  3000) ;
setTimeout( main_loop,  4000) ;
...
setInterval(() => {
  con.query( ' delete from moteinfo where tm < DATE_ADD( now() , interval -14 HOUR)',
          (err,res) => { if(err) console.log(err); } ) ;
}, 600000) ;
